"""
prototype_stage06_remerge.py

Stage 6 prototype for the 2.5D Mamba-hybrid architecture.

This validates the Stage 6 re-merge operation:

    Stage 5 Mamba2 output:
        (B, Z, D_MODEL)

    Stage 6 re-merged z-context:
        (B * Z, D_MODEL)

This is the inverse of Stage 4. It restores the merged slice-row layout so
that the z-context can later be broadcast spatially and fused with the Stage 2
bottleneck in Stage 7.
...
Run on the server:
    ...
    python prototype_stage06_remerge.py
...
- If CUDA or mamba_ssm is unavailable, this script bypasses Mamba2 with an
  identity path so the Stage 6 reshape logic can still be smoke-tested.
"""
...
def reshape_sequence_to_pooled(seq: torch.Tensor) -> torch.Tensor:
    ...
    This is the inverse of Stage 4.
...
def main() -> None:
    ...
    print("=" * 80)
    print("Stage 6 prototype: re-merge Mamba2 output for fusion")
    print("=" * 80)
    ...
    if use_mamba:
        ...
        # hard-coding Mamba1-style assumptions unless specifically validated.
        mamba_block = Mamba2(d_model=D_MODEL).to(device)
        mamba_block.eval()

        num_params = sum(p.numel() for p in mamba_block.parameters())
        print(f"    Mamba2 parameters: {num_params}")
    ...
    #     (B * Z, D_MODEL)
    #
    # This restores the merged slice-row layout used by the spatial bottleneck:
    #
    #     flat[b * Z + z] == z_context[b, z]
    #
    # Stage 7 will then broadcast each row vector over (BOTTLENECK_X,
    # BOTTLENECK_Y) and concatenate it with the Stage 2 bottleneck.
    # -------------------------------------------------------------------------
    ...
    assert z_flat.ndim == 2, (
        f"Stage 6 output must be 2D: (B * Z, D_MODEL). Got {z_flat.ndim}D."
    )

    assert z_flat.shape[0] == z_context.shape[0] * z_context.shape[1], (
        "Stage 6 must merge batch and sequence dimensions. "
        f"Expected {z_context.shape[0] * z_context.shape[1]} rows, "
        f"got {z_flat.shape[0]}."
    )

    assert z_flat.shape[1] == D_MODEL, (
        f"Stage 6 channel dimension mismatch. Expected D_MODEL={D_MODEL}, "
        f"got {z_flat.shape[1]}."
    )

    # -------------------------------------------------------------------------
    # Verify Stage 6 row order.
    #
    # For every b and z:
    #     z_flat[b * Z + z] == z_context[b, z]
    # -------------------------------------------------------------------------
    print("    verifying Stage 6 row order...")
    for b in range(B):
        for z in range(Z):
            row = b * Z + z

            assert torch.equal(z_flat[row], z_context[b, z]), (
                f"Stage 6 row-order mismatch: b={b}, z={z}, row={row}"
            )

    print("    Stage 6 row order verified: flat[b * Z + z] == z_context[b, z]")

    # -------------------------------------------------------------------------
    # Stage 6 round-trip canary.
    #
    # Use an identifiable sequence tensor to prove:
    #
    #     (B, Z, D_MODEL)
    #         -> (B * Z, D_MODEL)
    #         -> (B, Z, D_MODEL)
    ...
    # -------------------------------------------------------------------------
    print("    verifying Stage 6 round-trip canary...")
    ...
    assert flat_ids.shape == (B * Z, D_MODEL), (
        f"Stage 6 canary flat shape mismatch. Expected {(B * Z, D_MODEL)}, "
        f"got {tuple(flat_ids.shape)}."
    )
    ...
    for b in range(B):
        for z in range(Z):
            ...
            assert torch.equal(flat_ids[row], seq_ids[b, z]), (
                f"Stage 6 canary row-order mismatch: b={b}, z={z}, row={row}"
            )

    seq_ids_back = flat_ids.reshape(B, Z, D_MODEL)

    assert seq_ids_back.shape == seq_ids.shape, (
        f"Stage 6 canary round-trip shape mismatch. "
        f"Expected {tuple(seq_ids.shape)}, got {tuple(seq_ids_back.shape)}."
    )

    assert torch.equal(seq_ids, seq_ids_back), (
        "Stage 6 round-trip failed: reshaping back to (B, Z, D_MODEL) "
        "did not recover the original sequence tensor element-wise."
    )

    print("    Stage 6 round-trip canary verified.")

    print("=" * 80)
    print("Stage 6 prototype passed.")
    print("Next step: Stage 5b AMP validation for Mamba2:")
    print("    autocast fp16 forward/backward with GradScaler")
    print("=" * 80)
