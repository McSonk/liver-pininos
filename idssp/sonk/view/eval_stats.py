...
def plot_test_boxplots(df: pd.DataFrame, model_name: str = "Model", attach_hd95: bool=False):
    ...
    IoU columns are optional; if missing, the IoU subplot is skipped gracefully.
    """
    ...
    if attach_hd95:
        # Extract HD95 data (required when attach_hd95=True)
        ...
        if has_iou:
            ...
            _draw_metric_boxplot(ax3, [iou_liv, iou_tum], [COLOR_LIVER, COLOR_TUMOUR],
                                f'{model_name} Test Set: Intersection over Union',
                                'IoU Score', y_lim=[-0.05, 1.1])
        else:
            # Fallback to 2-row layout when IoU columns are absent
            logger.warning(
                "IoU columns ('iou_liver', 'iou_tumour') not found in DataFrame. "
                "Skipping IoU subplot."
            )
