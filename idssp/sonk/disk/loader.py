...
class DataCollector:
    ...
    def _calc_and_write(self, per_case_csv: Path, output_file: Path, train_ratio: float=0.8) -> tuple[list, list]:
        ...
        ------
        `per_case_csv`: Path
            The path to the per-case statistics CSV file, which must include the
            "tumor_to_liver_ratio" column. This file is generated by analyse_dataset.py.
        `output_file`: Path
            The path to the JSON file where the split will be logged.
        `train_ratio`: float
            The ratio of training samples in the split (default: 0.8).
        
        '''
        if not per_case_csv.exists():
            raise FileNotFoundError(
                f"per_case_summary.csv not found at {per_case_csv}. "
                "Run analyse_dataset.py first."
            )

        logger.info("No existing split log found. Calculating new stratified split...")

        stats = pd.read_csv(per_case_csv)
        ...
        logger.info("Split logged to %s. Next time you run the script, it will use this split.", output_file)
        return train_files, val_files
